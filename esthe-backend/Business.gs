...
const EVAL_KEYS=['recordType','visitDetails','authSubject','authEmail','opinionReaction','opinionWords','soilReaction','soilWords','scopeReaction','scopeWords','hookWhere','hookWords','guardWhere','guardWords','refusalWords'];
...
function isScriptSheet_(name) { return name==='スクリプト'||/^スクリプト(?:V\d.*|[_ -].+)$/.test(name); }
...
function getData(options) {
 ...
 const scriptSets={};ss.getSheets().filter(s=>isScriptSheet_(s.getName())).forEach(s=>{scriptSets[s.getName()]=s.getRange(1,1,Math.max(s.getLastRow(),1),Math.max(2,Math.min(s.getLastColumn(),4))).getDisplayValues();});
...
function validate_(p){
 ...
 if(!['アポ獲得','再電話','不在','資料送付','後日突撃','継続フォロー','保留・時期待ち','対応完了','見送り','連絡不要','訪問済'].includes(p.status))throw new Error('結果を選択してください。');
...
function saveRecord(p) {
 ...
 if(!p.scriptSheet||!isScriptSheet_(p.scriptSheet)||!ss.getSheetByName(p.scriptSheet))throw new Error('使用スクリプトが見つかりません。');
 ...
 if(p.recordType!=='訪問')list.sh.getRange(target.row,list.heads.indexOf('TEL日')+1).setNumberFormat('@').setValue(displayDate);
 ...
 return {date:p.recordType==='訪問'?nowDate:displayDate,status:p.status};
